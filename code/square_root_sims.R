## apply signal on log-scale, analyze on square root scale.

one_rep <- function(new_params, current_params) {
  source("nc_adjustment_methods.R")
  source("non_nc_methods.R")
  ...
  ## VOOM LIMMA EBAYES -------------------------------------------------------
  vout              <- limma::voom(counts = t(d_out$Y), design = X)
  lout              <- limma::lmFit(vout)
  eout              <- limma::ebayes(lout)
  method_list$limma <- list(betahat = lout$coefficients[, 2], sebetahat = sqrt(eout$s2.post) * lout$stdev.unscaled[, 2],
                            df = eout$df.total[1], pvalues = eout$p.value[, 2])

  ## DESEQ2 ------------------------------------------------------------------
  suppressMessages(ddat <- DESeq2::DESeqDataSetFromMatrix(countData = t(d_out$Y), colData = as.data.frame(X), design = ~ Treatment))
  suppressMessages(deseq2out <- DESeq2::DESeq(object = ddat))
  ddresults <- DESeq2::results(deseq2out)
  method_list$deseq2 <- list(betahat = ddresults$log2FoldChange, sebetahat = ddresults$lfcSE, df = Inf, pvalues = ddresults$pvalue)
...
mat <- t(as.matrix(read.csv("~/Code/rcc_copy/projects/reproduce_mouthwash/Output/gtex_tissue_gene_reads_v6p/muscle.csv",
                            header = TRUE)[, -c(1,2)]))
...
saveRDS(cbind(par_vals, sout), "../output/square_root_sims_out.RDS")
