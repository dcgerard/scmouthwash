...
I try the same as code as in [here](bulk_explore.html) use +/-2 as the signal, as in Joyce's work.
...
write.csv(result_mat, "../output/bulk_bimodal_results2.csv", row.names = FALSE)
```

```{r}
result_mat <- read.csv("../output/bulk_bimodal_results2.csv")
```
