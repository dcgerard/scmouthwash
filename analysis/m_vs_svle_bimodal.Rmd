...
Same as m_vs_svle but using the bimodal signal dataset. I do see the same results as Joyce. My version of ASH is better calibrated, though.
...
```{r, cache=TRUE}
num_dat <- length(counts_allgenes_bimodal)
...
write.csv("../output/sc_joyce_bimodal_results.csv")
```
...
The bad AUC performance is entirely driven by datasets where most of the genes are non-null! In the bulk RNA datasets, I only ever explored scenarios with $pi_0 > 0.5$, so this might also occur in bulk data.

MOUTHWASH has higher AUC for datasets with more than 50\% null
```{r}
filter(longdat, true_pi0 >= 0.5) %>%
  ggplot(mapping = aes(x = method, y = auc)) +
  geom_boxplot()
```
